...
- **温度（temperature）**：可用。**数值留空 = 系统默认（placeholder 直接显示具体
  默认值，如 1.0），填写 = 覆盖**；数值框隐藏上下箭头、仅输入数字。经官方
  `agent/request` 钩子注入（新会话首请求解析「留空 ? 默认 1.0 : 全局温度」，按
  会话钉住并保持固定；`agent/disposed` 清理会话态）。零 dsh 源码修改，与重试/
  推理补齐同层的官方扩展点。
