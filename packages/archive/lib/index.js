/**
 * better-webui archive host half: archived-session management.
 *
 * A plain function plugin (no build step) owning exactly one domain: the
 * archived-session surface the native UI lacks. It registers one dedicated
...
function where(...parts) {
  return parts.filter((part) => part !== undefined && part !== null && part !== '').join(' ') || 'better-webui-archive'
}
...
/**
 * Register the `/better-webui` RPC channel and self-heal dead references left
 * by earlier deletes or manual cleanups (ids accounted or archived for
...
export function apply(ctx) {
  ...
  ctx.effect(() => handle, 'better-webui-archive: rpc channel')

  handler('purge', {}).then((result) => {
    if (result.ok !== true) return
    const { purged, detached } = result.value
    if (purged.length > 0 || detached.length > 0) {
      console.warn(`better-webui-archive boot sweep: purged archive ids [${purged.join(', ')}], detached accounting ids [${detached.join(', ')}]`)
    }
  }).catch(() => {
