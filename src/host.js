        ...
        try {
          ...
          if (terminals !== undefined && typeof terminals.list === 'function' && typeof terminals.kill === 'function') {
            ...
            if (reset) {
              ...
              console.warn(`better-webui: persistent-bash stall guard reset the shell after ${STALL_STRIKES} consecutive slow bash calls (each ≥ ${STALL_THRESHOLD_MS}ms)`)
            }
          }
        } catch (error) {
          console.warn(`better-webui: persistent-bash stall guard tracking failed: ${error instanceof Error ? error.message : String(error)}`)
        }
      }
    }
    return result
  })
  ctx.effect(() => dispose, 'better-webui: persistent-bash stall guard')
}
...
export function apply(ctx) {
  ...
  // Persistent-bash stall guard: watch for consecutive silence-tier settles
  // (each ≥ STALL_THRESHOLD_MS) on an owner with live terminal sessions and
  // reset the shell so the degraded PTY readiness protocol recovers. The
  // `tools/execute` waterfall is always available when tools dispatch; the
  // `terminals` service is optional, and the installer no-ops without it.
  if (typeof ctx.on === 'function') {
    try {
      installPersistentBashStallGuard(ctx)
    } catch (error) {
      console.warn(`better-webui: persistent-bash stall guard failed to install: ${error instanceof Error ? error.message : String(error)}`)
    }
  }
}
