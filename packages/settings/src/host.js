/**
 * better-webui settings host half: configurable LLM retry policy + dedicated
 * better-webui settings page backend.
 *
 ...
 *     restart needed.
 *
 * Wire: the browser half calls `ctx.connection.rpc.call('/better-webui-settings',
...
export const CHANNEL = '/better-webui-settings'
...
function where(label) {
  return label === undefined || label === null || label === ''
    ? 'better-webui-settings'
    : `better-webui-settings ${label}`
}
...
/**
 * The `/better-webui-settings` channel handler table.
 * @param {import('@deepseek-ai/cordis').Context} ctx - host plugin context.
...
/**
 * Register the `/better-webui-settings` RPC channel and the `better-webui`
...
export function apply(ctx) {
  ...
  ctx.effect(() => handle, 'better-webui-settings: rpc channel')
  ...
  if (typeof ctx.root?.on === 'function') {
    ...
    ctx.effect(() => disposeSettingsWatcher, 'better-webui-settings: settings watcher')
  }
}
